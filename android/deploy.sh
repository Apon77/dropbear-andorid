...
adb push dropbear dropbearkey dbclient scp "$D/"
adb push "$PUBKEY" "$D/ak/authorized_keys"
adb shell "test -f $D/hostkey" || adb shell "$D/dropbearkey -t ed25519 -f $D/hostkey"
adb shell "chmod 755 $D/dropbear $D/dropbearkey $D/dbclient $D/scp; ln -sf $D/dbclient $D/ssh; chmod 700 $D $D/ak; chmod 600 $D/ak/authorized_keys $D/hostkey"
